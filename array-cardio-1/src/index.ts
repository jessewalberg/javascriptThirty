...
const inventorsSortedOldestFirst = inventorsCopy.sort(
  (a, b) => b.year - a.year
);

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const yearsInventorsLivedTogether = inventors.reduce(
  (prev, curr) => prev + (curr.passed - curr.year),
  0
);

// 5. Sort the inventors by years lived
const inventorsCopyYearsLived = [...inventors];
inventorsCopyYearsLived.sort((a, b) => {
...
const dataSummed: StringKeyedObject = data.reduce(
  (prev: StringKeyedObject, curr: string) => {
    if (!prev[curr]) {
      prev[curr] = 0;
    }
    prev[curr]++;
